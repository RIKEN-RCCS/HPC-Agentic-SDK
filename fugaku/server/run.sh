#!/bin/bash
# Launch an MCP server module from the fugaku_mcp package.
# Usage: run.sh <module>   e.g. run.sh fugaku_mcp.hpc_server
set -euo pipefail
...
if ! "$VENV/bin/python" -c "import fugaku_mcp, mcp, hpc_agent_core.mcp_server" >/dev/null 2>&1; then
